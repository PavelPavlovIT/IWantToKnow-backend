...
public class StudyBySpeakRepositoryDifficultLevel(
    IConfiguration configuration,
    ILogger<StudyBySpeakRepositoryDifficultLevel> logger,
    IDataService dataService,
    ApplicationDbContext context) : BaseRepository(logger, dataService), IStudyBySpeakRepositoryDifficultLevel
{
    private async Task<StartTestViewModel?> GetNextQuestionForStudyAsync(ApplicationDbContext context, string userId, string categoryId, CancellationToken cancellationToken)
    {
        ...
        List<NextQuestionRecord>? firstQuestionsRecord = await (
            from completedStudyQuestions in context.CompletedStudyQuestionsBySpeakDifficultLevel
        ...
        nextQuestionViewModel.ListOfAnswers = await GetCorrectAnswersForSpeakAsync(
            context,
    ...
    public async Task<StartTestViewModel?> StartStudyAsync(string lang, string userId, string categoryId, CancellationToken cancellationToken)
    {
        ...
        var completedStudyQuestionList = await (
                ...
                join categoryAndQuestions in context.CategoryAndQuestions
                    on questions.Id equals categoryAndQuestions.QuestionId
                join completedStudyQuestions in context.CompletedStudyQuestionsBySpeakDifficultLevel
        ...
        if (completedStudyQuestionList.Count == 0)
        {
            ...
            listQuestion.ForEach(async void (i) =>
                await context.CompletedStudyQuestionsBySpeakDifficultLevel.AddAsync(
                    new CompletedStudyQuestionBySpeakDifficultLevel()
                    {
    ...
    {
        var result = request.SpeakingResult;
        
        if (result)
        {
            var item = await context.CompletedStudyQuestionsBySpeakDifficultLevel
