...
public class StudyByListenRepository(
    IConfiguration configuration,
    ILogger<StudyByListenRepository> logger,
    IDataService dataService,
    ApplicationDbContext context) : BaseRepository(logger, dataService), IStudyByListenRepository
{
    private async Task<StartTestViewModel?> GetNextQuestionForStudyAsync(ApplicationDbContext context, string userId, string categoryId, CancellationToken cancellationToken)
    {
        ...
        List<NextQuestionRecord>? firstQuestionsRecord = await (
            from completedStudyQuestions in context.CompletedStudyQuestionsByListen
    ...
    {
#if DEBUG
        logger.LogInformation("StartStudy");
        logger.LogInformation("lang {0}", lang);
        logger.LogInformation("userId {0}", userId);
        logger.LogInformation("CategoryId {0}", categoryId);
#endif
        ...
        var completedStudyQuestionList = await (
                ...
                join categoryAndQuestions in context.CategoryAndQuestions
                    on questions.Id equals categoryAndQuestions.QuestionId
                join completedStudyQuestions in context.CompletedStudyQuestionsByListen
        ...
        if (completedStudyQuestionList.Count == 0)
        {
            ...
            listQuestion.ForEach(async void (i) =>
                await context.CompletedStudyQuestionsByListen.AddAsync(
                    new CompletedStudyQuestionByListen()
                    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (result)
        {
            var item = await context.CompletedStudyQuestionsByListen
